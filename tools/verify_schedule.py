...
# `X as X` is an explicit re-export: `main` does not use these three, and `tests/test_gates.py`
# reads all three through this module, so an unused-import fix would break the test, not tidy the
# gate. (The first cut dropped `NO_RESULT_YET` as unread; the test that reads it said otherwise.)
from swingdesk.platform.schedule import (
    CLEAN_RESULTS as CLEAN_RESULTS,
)
from swingdesk.platform.schedule import (
    DIAGNOSED as DIAGNOSED,
)
from swingdesk.platform.schedule import (
    HAZARDS,
    TASKS,
...
from swingdesk.platform.schedule import (
    NO_RESULT_YET as NO_RESULT_YET,
)
from swingdesk.platform.schedule import query_task as _query

#: The parser, the task list and every result code moved to `swingdesk.platform.schedule` on
#: 2026-09-12, so `swingdesk status` reads the scheduler with this gate's judgement rather than a
