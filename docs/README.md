...
| 67e | `08-pm/audits/2026-09-05-0f7c67a3d455/executive-report.md` | The complex code audit, run once report-only on a frozen master - three domain verdicts, seven non-negotiables probed independently, and one finding that overturned a claim this repository published the same day | drafting |
| 67d | `08-pm/plans/2026-09-05-what-watches-the-root-documents.md` | Three signals for a document-retirement finder, all measured dead - and the hole that does exist: the open-work list has been outside both document gates since they were written | owner-pending |
...
| 68 | `08-pm/TRIAL_BUDGET.md` | What the programme may spend against `b.deflated_sharpe`: the criterion was ratified with nothing counting its only input, 13 trials are already spent against a census that reads 5, and the hurdle grows logarithmically so rationing late buys almost nothing | frozen |
...
| 70 | `02-domain/STRATEGY_CONTRACT.md` | What a COMBINATION must declare before it may be called a strategy - ten clauses, each forced by a measurement: the null as a field, net with an interval, cost as an admission rule, the tail budget beside the mean, and a holding period that is the strategy's own and researched rather than inherited as a constant. `CARD-001` passes two and a half of the ten | drafting |
...
| 72 | `02-domain/CARD-002-overnight-small-caps.md` | The second strategy card and the first one a study earned: buy `IJR` and `VB` in the closing auction, sell both at the next open, every session. No selection, no trigger and no stop - the exit resting at the venue is the protection, because the position is held only while the exchange is shut. Its size is `unset` so it refuses, and the paper trial it asks for exists to measure the one thing `PR-034` could not: what the auctions give | drafting |
| 73 | `05-validation/RETURN_SOURCE_REGISTER.md` | Every SOURCE OF RETURN this project has touched, with what was measured and what would reopen it. Created by `DR-047` when 166 configurations and 465 components had produced zero validated strategies: the unit of research becomes a source with a mechanism and a literature, not an entry rule. Six families are closed with their triggers, so the deflated-Sharpe hurdle is not paid twice for the same ground | drafting |
