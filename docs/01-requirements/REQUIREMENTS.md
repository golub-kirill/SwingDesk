...
| `REQ-AI-001` | AI output MUST NOT bypass an independent risk engine, and a risk veto MUST NOT be overridable by the agent. | **NOT met, scope undecided** — this was recorded as deferred citing `CHARTER.md` §3, which does not mention AI anywhere (`COVERAGE_AUDIT.md` §4). The deferral had no basis |
| `REQ-AI-002` | An AI agent MUST NOT generate numeric quantities (win rate, probability, expectancy, score, stop, target, position size, weights, slippage, edge) from text. These MUST come from deterministic engines or a validated expectation estimate. | **NOT met, scope undecided** — as above |
