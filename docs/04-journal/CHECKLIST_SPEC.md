...
**Five of the eighteen are answerable today, not twelve.** The paragraph below says twelve are
...
missing — eight of them do. An `unavailable` item is a gap in the **system**, so it is shown beside
...
|---|---|
| E01 identity · **E02 universe membership** · E13 risk recomputed · E16 time stop · E17 no skip condition | E03 corporate actions · E04 regime not wired · E05 no sector source · E08 no trigger/max entry · E09 no maximum entry · E11 no event calendar · E12 no spread data · E14 no exposure buckets |

E02 moved when the universe path landed (ROADMAP X1), and it carries a distinction worth keeping:
the count above is what the **system** can answer, and a run that took an explicit ticker list still
reports E02 `unavailable`. Membership is a fact about a rule; typing a symbol is not evidence of one.
