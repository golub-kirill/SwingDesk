...
| **Portfolio & capital allocation** | `RISK_SPEC.md`, `registry/parameters.yml` | `PARTIALLY_COVERED` | the objects exist and are named: `risk.max_open_risk`, `risk.max_sector_risk`, `risk.correlation_threshold`, `risk.max_concurrent_positions`, `risk.max_position_value` — ~~**all five `unset`**~~ **all five carry a value and name a consumer since `DR-006` was fully ratified on 2026-08-23**, and the portfolio *layer* they were missing is `trade_management/portfolio.py`. What is still missing is a deterministic ranking when candidates exceed capital | ranking needs ТЗ §31, still `ABSENT`; `rs.ranking_method` and `rs.benchmark_form` are the two values it waits on |
