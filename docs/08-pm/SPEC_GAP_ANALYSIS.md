...
| 27 | Backtest Semantics | PARTIAL | `BACKTEST_PROTOCOL.md`, `validation/backtest/engine.py`; intrabar policy specified in `EXECUTION_MODEL.md` §4. ~~not yet enforceable — nothing can violate it while no target exists~~ — **a target has existed since 2026-09-01**: `exit.target_r_multiple` is owner-set at 1.0 and `broker/submit.py:target_price` reads it, and `validation/backtest/intraday.py` resolves a same-bar stop and target through `first_touch` and `break_tie`. Corrected 2026-09-21, twenty days stale |
...
| 29 | Order Management SM | **PARTIAL** | ~~DEFERRED — D1, the system never places orders~~ — **it has placed them since 2026-09-01** (`broker/alpaca.py:submit`, to the paper venue, behind `CHARTER` A-002 and four guards). `DR-050` then found the CLOSE half of it recorded no stop-out for as long as it existed, which is what an order-management contour graded DEFERRED buys you. Corrected 2026-09-21 |
