...
|---|---|---|---|
| D-1 | ~~**Survivorship can never be corrected.** No free source serves delisted instruments~~ — **REFUTED 2026-09-05: Alpaca serves delisted daily paths from 2016 on `feed=sip`, and the owner ruled the account is free tier. THE RISK STAYS OPEN AND ITS SEVERITY IS UNCHANGED, because a route nobody has taken corrects nothing** — no study has used it. What changed is the word *never*. Owner decision D10 (2026-08-02) reaffirmed the free tier believing this uncorrectable, and the free tier turned out to carry the route | **high / accepted** | Mandatory disclosure field on `EvidenceRecord`, unskippable by construction. Quantified per study — PR-002 needs only ~2% of trades missing at −2R to lose its finding |
...
| D-3 | **No POINT-IN-TIME Canadian membership, and no Canadian bars.** This row read *"Canada cannot be enumerated. No free `.TO` symbol directory in hand"* and was rated **high / accepted** until 2026-08-25, when the enumeration half was **refuted**: TMX serves its own listed-company directory free, no account, no key — derive it with `python tools/probe_canada.py --full` (`DR-003` addendum). What survives is narrower and is two distinct facts: the endpoint serves **today's** directory, so applying it to old data is the survivorship bias `DR-003`'s own alternatives table objects to for index membership; and the Canadian half of the bar store is empty, which is a coverage question the probe does not ask | **medium / accepted** — the rating fell with the refutation, and the residual is a point-in-time and coverage risk rather than an enumeration one | Every result reported so far is single-market and said so. `BR-9`'s per-country requirement is **still unmet** — no study has been re-run — but it is no longer unmeetable, which is the change: a future study can carry the two-country requirement instead of declaring it impossible |
