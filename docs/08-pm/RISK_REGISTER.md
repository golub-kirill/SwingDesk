...
| E-5 | **Multiple comparisons.** Four studies plus a post-hoc bound, several arms each, no correction applied | **medium** | Stability requirements (cost stress, holdout, second null) instead of an adjusted p-value. Defensible and not equivalent; `PREREG_TEMPLATE` §6 records the debt |
...
| ID | Risk | Rating | Control |
|---|---|---|---|
| B-1 | **Solo project, no second reviewer.** Every finding above was caught by a machine because there is nobody else to catch it | **certain / structural** | 23 merge gates from one command. **Gates 13 and 15 are tested for the ability to fail** (`tests/test_gates.py`, 10 cases against fixture trees); the other seventeen are not, and this row said otherwise until 2026-08-05 |
