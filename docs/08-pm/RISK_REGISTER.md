...
| E-5 | **Multiple comparisons.** Three studies plus a post-hoc bound, several arms each, no correction applied | **medium** | Stability requirements (cost stress, holdout, second null) instead of an adjusted p-value. Defensible and not equivalent; `PREREG_TEMPLATE` §6 records the debt |
