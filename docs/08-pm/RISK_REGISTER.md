...
|---|---|---|---|
| D-1 | **Survivorship cannot be corrected, and half of why was wrong.** ~~No free source serves delisted instruments~~ — **amended 2026-08-25, one day after `tools/probe_edgar.py` refuted it and `VENDOR_COMPARISON.md` §7 and `EVIDENCE_SUMMARY.md` §3 were corrected — this row was not, which is the citation sweep `AGENTS.md` §12 asks for and nobody ran.** SEC EDGAR serves the FACT and DATE of a delisting free and official back to 1993 (`tools/probe_edgar.py`, `VENDOR_COMPARISON.md` §7); no free source serves the delisted name's PRICE PATH, which is what a backtest needs to compute its return. So the exposure's SIZE is obtainable and its MAGNITUDE is not, and the risk stands on the second half alone. Owner decision D10 (2026-08-02) reaffirmed the free tier knowing this | **high / accepted** | Mandatory disclosure field on `EvidenceRecord`, unskippable by construction. Quantified per study — PR-002 needs only ~2% of trades missing at −2R to lose its finding. **That ~2% is still an assumption and is now the kind of thing that could be measured**: `TODO.md` §2 has the route and it waits on one owner action, `SWINGDESK_EDGAR_CONTACT` |
...
| D-3 | ~~**Canada cannot be enumerated.** No free `.TO` symbol directory in hand~~ **REFUTED 2026-08-25 — the CAUSE moved and the consequence did not.** TMX serves its own listed-company directory free, no account, no key (`DR-003` *"Gap 1 is closed"*; derive the population with `python tools/probe_canada.py --full`, never from this row). What blocks a Canadian result now is **bar coverage** — the Canadian half of the store is empty — and point-in-time membership, which the endpoint cannot supply at any price. The rating is unchanged because what it rates is the consequence | **high / accepted** | Every result is still reported single-market and `BR-9`'s per-country requirement is still unmet, said so rather than quietly dropped. **What changed is that the blocker is now a fetch nobody has run rather than a source nobody has**, and `PR-001`, `PR-002`, `PR-005` and `PR-008` all narrowed their own scope on it — five of their eight citations kept `DR-003`'s *"in hand"* qualifier and three turned it into an unqualified *"cannot"* (`PR-001` §10, third amendment, counts them) |
...
| ID | Risk | Rating | Control |
|---|---|---|---|
| B-1 | **Solo project, no second reviewer.** Every finding above was caught by a machine because there is nobody else to catch it | **certain / structural** | The merge gates run from one command (`HANDOFF.md` §2 carries the count). **Some gates are tested for the ability to fail** — `tests/test_gates.py` builds fixture trees and asserts the gate reports *that* defect; most are still not, and this row said otherwise until 2026-08-05 |
