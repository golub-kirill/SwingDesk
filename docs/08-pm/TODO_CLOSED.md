...
**Status:** drafting · **Tier:** 8 (PM) · **Opened:** 2026-09-05

**What this is.** Work items from `TODO.md` that are finished. They are here rather than deleted
because the *"what paid for it"* paragraphs are the most re-read thing this project writes, and
they are here rather than in `TODO.md` because `AGENTS.md` §10.7 gives that file one job: **every
open item, and only open items.**

**What it is NOT.** Not a work list, not a plan, and not a record anything cites. `AGENTS.md` §11
rule 2 protects decisions, ADRs, ratified criteria, pre-registrations, reports, journal entries and
evidence — a finished work item is none of those. Nothing here is load-bearing: the 123 citations
of `TODO.md` across this repository all point at SECTIONS, none at an individual item.

**Read `TODO.md` first.** If an entry here contradicts one there, `TODO.md` is right and this is
history.

**How an item arrives.** Its lesson is promoted — to `AGENTS.md` §12's traps, to a decision record,
or to a gate — and then the entry moves. A lesson that is only *"this specific thing was fixed"*
is not promoted, because git holds it. **Gate 43 refuses a closed item in `TODO.md`**, which is
what keeps this from drifting back.

**The first 105 arrived together**, on 2026-09-05, when the rule and the file were reconciled.
...
- [x] **`[v]` New research suspended — 2026-08-16, same council, unanimous.** No new
      ...
      **OVERRIDDEN BY THE OWNER 2026-08-24 for a single study, and the override is recorded rather than
      ...
      **Second, the owner's reason is a constraint the council did not have.** Six months, and at
      ...
      than the horizon.
      **Still suspended:** everything else on the list above. This override names its study and
      covers no other.
...
- [x] **`[v]` ~~`PR-011` — screening out the instrument classes that cannot hold a stop — IS NOT
      ...
      **Three things the writing established that reading would not have.**
      ...
      Migrated here 2026-08-22 from the dated session handoff of that day, §3, before that
      file
...
- [x] **`[v]` ~~THE COVERAGE TIER WAS SPECIFIED AND NEVER SCHEDULED, AND THE UNIVERSE IS 28%
      ...
      **Gate 26 passes on it, and for the right reason rather than by accident.** It reports the
      ...
      result is `267011` = `0x00041303` = `SCHED_S_TASK_HAS_NOT_RUN`~~ — **IT HAS RUN NOW.**
      Owner instruction 2026-09-04, *run it by hand before the schedule reaches it*, and it
      was triggered through the registered task itself (`schtasks /Run`) rather than by
      calling the wrapper, so the REGISTRATION is what was exercised and not just the script.
      **`exit 0`, 23:45:22 → 23:55:10.** `fetched 3784, failed 216` of the 4,000-symbol
      default budget, and the tool's closing lines report the coverage and that one more pass
      at this budget covers the directory. The 216 are the expected class — warrants, units
      and rights map to no vendor symbol — which is why the tool prints both counts.
      **Gate 26 now judges a real exit code** and drops its *"has not run yet"* note.
      **One number in `widen_universe.cmd`'s own comment does not survive this run, and the
      population is why — not the arithmetic.** The comment budgets 45 seconds per 100
      symbols; this pass did 4,000 in 588 seconds, about 15. Both are honest: the 45 was
      measured against symbols with NO stored bars, needing a full two-year fetch, and after
      the catch-up most of these had bars and took an incremental one. **The estimate is
      right for the population it was measured on and wrong for this one**, which is the
      thing to carry rather than a corrected constant.
      **Neither coverage figure in this entry is quotable any more** — two hand-run passes
...
- [x] **~~GATE 33 REPORTS EIGHT OVERLAPS THAT CANNOT EXIST, AND IT WILL DO SO FOREVER~~ — found
      2026-09-04, CLOSED 2026-09-04 on the owner's instruction. It reports **zero** now.** `tools/verify_sibling_edits.py` names `claude/a-research-instrument-not-a-broker`
      ...
      **So the alarm is now 100% false, which is worse than it was when this entry was
      ...
      efforts rewriting one paragraph.

      **RULED AND DONE 2026-09-04 — the owner said delete both, and the removal is recorded
      here rather than merely done** (`AGENTS.md` §11 rule 6):
      ```
      git branch -D claude/a-research-instrument-not-a-broker   # was d406a39
      git branch -D claude/the-tool-missed-a-guard              # was aa3c0ce
      ```
      `tools/verify_sibling_edits.py` reports **0 overlaps** immediately afterwards, against the
      six it reported that morning. The guard is quiet again, which is the only state in which
      an operator will read it on the evening it matters.
      **~~both are recreatable from `origin/`~~ — HALF WRONG, and checking it is what stopped
      this being a real loss.** `claude/the-tool-missed-a-guard` is: local and remote were both
      `aa3c0ce`. `claude/a-research-instrument-not-a-broker` was **not** — origin's tip is
      `14546bd` and `d406a39` is not an ancestor of it, so the local ref was orphaned and
      `git checkout -b … origin/…` would have returned a different branch, quietly.
      **What made the deletion safe is a blob comparison, not the remote.** Four of the five
      files `d406a39` touched are **byte-identical** to `master`'s `78f5ca0`; the fifth is
      `HANDOFF.md`, and the difference is its generated state block. Nothing unique was in that
      commit. `origin/claude/a-research-instrument-not-a-broker` also carries **0** commits
      `master` lacks, so the remote ref is itself fully merged.
      ```bash
      git merge-base --is-ancestor d406a39 14546bd     # returns non-zero: it is NOT
      git rev-parse d406a39:tools/verify_registry_keys.py 78f5ca0:tools/verify_registry_keys.py
      ```
      **The lesson is the entry's own, turned on itself:** *"the remote ref survives either
      way"* was written about one branch and applied to a class, which is the impossibility
      shape (§15) with the sign flipped — a claim that something is SAFE, asserted rather than
      tested. It cost nothing here only because it was checked before the command ran.
...
- [x] **`[v]` ~~MEASURED 2026-08-17: 3 of 11 mutants survive the entire test suite.~~ RE-MEASURED
      ...
      **Killed (properly asserted):** `exits.py` stop distance and holding-period comparison,
      `contracts/position.py` `open_risk`, `derived_observations/atr.py` true range,
      `derived_observations/pivots.py` confirmation lag, `reference_data/universe.py` ADTV
      window and `vendor_symbol`, `reference_data/directory.py` departure set.
