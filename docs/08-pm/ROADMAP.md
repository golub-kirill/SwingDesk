...
| `UDR-004` | Canonical regime ontology | **OPEN, and sharper than recorded.** The ТЗ suggests eight regimes; the course names **eleven** (`REGIME_SPEC.md` §2) and they are a vocabulary, not a partition. PR-002 ~~validated~~ **fitted** a classifier on one axis of three, and its verdict was corrected to `INCONCLUSIVE` on 2026-08-16 — `regime.classifier_rule` is `assumed:PR-002` and this project has no `validated` parameter. The real question is whether the ТЗ list or the course list is canonical — and only the course list has any evidence behind it at all |
