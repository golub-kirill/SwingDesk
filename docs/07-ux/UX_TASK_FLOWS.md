...
| USA/Canada regime determined | **no**, and the two halves fail for different reasons. The classifier exists (M30-T0450) and is **not wired into the run** — that half is unchanged. The Canadian half read *"Canada cannot be enumerated at all (`DR-003` gap 1)"* until 2026-08-25, when that was **refuted**: TMX serves its listed-company directory free (`python tools/probe_canada.py --full`, `DR-003` addendum). What blocks Canada now is **coverage, not enumeration** — the Canadian half of the bar store is empty, and the endpoint serves today's membership rather than a point-in-time one |
| sector map, commodities, CADUSD | **part**, and this row read a flat **no** until 2026-08-25. `Instrument.sector` is indeed still `None` and nothing sets it — but the conclusion drawn from that was wrong: the sector source **is** in hand and is enforced. `DR-006` §12 (2026-08-23) built `ClassificationStore`, keyed on `knowledge_time` so it is read as-of, fed by the bar vendor's own classification and ETF look-through, and `trade_management/portfolio.py:sector_limit` refuses on it in the live path. The live path reads the STORE, never `Instrument.sector`, which is why the field being `None` said nothing about the source. Derive the coverage with `python tools/measure_sector_cap.py --wide --classifications data/classifications.duckdb`, never from this row. **Commodities and CADUSD are unchanged: no FX series, and `account.fx_rate_cad` is `unset`** |
