...
| 3f | `verify_studies.py` | a report with no pre-registration; the prereg index disagreeing with the report it points at; a `validated:` parameter citing a study that did not ACCEPT; a `\| Studies \|` row whose numbers do not match the reports on disk | **exists** — caught a study census quoting a refuted total the reports on disk did not support, in five documents at once. *(The figures it caught are deliberately not repeated here: a numeral beside the word it counts is a live claim to this gate, whatever tense the sentence is in — `AGENTS.md` §10.5.)* |
...
| 6 | `lint-imports` | a package importing across a layer or forbidden boundary | **exists** — 5 contract subjects across 4 contracts. Caught a reversed layer order on first run. **And it was not being RUN: from the day the suite was wired until 2026-08-31 the runner invoked it as `python -m importlinter.cli lint-imports`, which imports a Click group with no `__main__` guard, falls off the end and exits 0** — so every suite run printed `PASS` over a check that never executed. Found by planting a forbidden import and watching it stay green (`AGENTS.md` §10.8). The tree turned out to be clean when the real runner was finally used — 4 kept, 0 broken — but that was luck, not enforcement. **The first fix was also wrong and CI is what said so**: resolving the console script as a sibling of `sys.executable` is true in a venv and false on the hosted Python the runner uses, where scripts live in `Scripts/` and the interpreter does not. Green locally, red remotely — the mirror image of `AGENTS.md` §12's first trap. Now `check_gates.LINT_IMPORTS` calls Click's command object through the same interpreter: no path to guess, no `PATH` to consult, and a failed import raises instead of exiting 0 |
...
| 20 | `verify_decisions.py` | an accepted decision record that names no implementation and does not say `implementation: none`, **or whose token appears only in a comment or a docstring** — **caught a false implementation claim on its first run**, and on 2026-09-03 caught `DR-011` citing its own id in prose, which the substring test it replaced had accepted for the four days since that record was ratified |
...
| 28 | `verify_parameter_claims.py` | a document **or a docstring** stating a parameter status the registry contradicts — **six live on the day it was written, every one a parameter that gained a value while the prose still called it `unset`**. **Widened 2026-09-05 after it missed seven more, and both blind spots were inside it:** it read only markdown, though the worst instances were docstrings (`pipeline.py` described the live decision path as refusing every candidate for nineteen days after `DR-012` stopped it), and it excluded the words *stays* and *remains*, which assert the present state rather than naming one end of a move. Scope went 88 files to 315. It does not police its own runner or its own fixtures — a gate red on its own evidence is measuring its instrument |
...
| 33 | `verify_sibling_edits.py` | two live branches rewriting the same lines. Overlaps are computed in **merge-base coordinates**, so a hit means both trees changed the same original text rather than merely the same file. **A deletion counts as a change to every line the file had** (2026-09-04), so one branch removing what another rewrites is reported — the collision a textual merge settles silently; a file both sides removed is not. Advisory, and **it did not run in CI** — a shallow clone has no sibling branches and it says so — **written the day gate 16 was green and two trees corrected the same two table rows two hours apart** |
...
| 38 | `verify_gate_citations.py` | a document naming a gate number that neither the runner registers nor this table lists. Gate 36 closes the two places a gate is DEFINED; this closes the ~130 documents that CITE one, which is where a reader actually meets a gate number and reads it as protection. **Written out of row 12**, which said `exists` for seventeen days over a number nothing runs. The vocabulary is deliberately THIS TABLE and not the runner: a row marked `to build` still makes the number citable, which is why the honest references to the unbuilt gate 10 are not findings (**~~twenty-odd~~ — re-derived 2026-09-05, eleven documents name it and eight state it as unbuilt; the same wrong figure stood in `TODO.md`, which is why both were corrected**). Measured before building - 363 citations, 0 unresolved, so prevention rather than repair - after excluding two forms that are not gate ids at all: a four-digit number, and a number followed by a hyphen. Both came from one real false positive, a date |
| 39 | `verify_broker_policy.py` | the broker adapter reaching a host the committed policy does not allow, or carrying an HTTP write verb. Gate 22's instrument aimed at a target where it matters more: Alpaca's account object names **no paper/live flag** - checked against its own API reference 2026-08-31, `id`, `status`, `currency`, `equity` and `trading_blocked` are identical in both - so **which host was called is the only thing separating play money from the owner's**, and `DR-014` rests on it. The allowlist carries exactly one https host, the live venue is named under `forbidden_hosts` and compared as a HOSTNAME rather than a substring (`paper-api.alpaca.markets` contains `api.alpaca.markets`, and this gate's first run failed on it), and `src/swingdesk/broker/` is read from the syntax tree for any URL or any of POST/PUT/PATCH/DELETE. and **exactly two functions may reach the transport**. **The verb rule survived the package gaining the ability to write** (`CHARTER` A-002, `DR-027`, 2026-09-01), because the verb is read from the policy via `BrokerPolicy.write_method` rather than spelled in code - a policy narrowed back to `GET` leaves the adapter with nothing to send instead of a literal it ignores. The one exemption is by NAME and not by file: `policy.REFUSED_METHODS`, a denylist, which is the opposite of a capability. **The transport check is the one with teeth now** - `_write` is where `DR-027` 4's four guards are consulted, and a second write path would consult none of them | **exists** - built 2026-08-31 with `ADR-0005` and `DR-026`, extended 2026-09-01 with `DR-027`. Every check was measured by planting the defect and watching it go red (`AGENTS.md` 10.8): a `POST` literal, a second transport call site, and the substring host comparison this gate found in itself on its first run |
| 40 | `verify_registry_keys.py` | a duplicate key inside one YAML mapping in `registry/`. YAML keeps the LAST one and drops the other **without a word**, so whatever sits above is invisible to every tool that loads the file while remaining in plain sight to a human reading it. **Both instances were found on 2026-09-01 and both were load-bearing**: `screen.trend_definition` lost *"STAYS UNSET BY EVIDENCE, and the family is CLOSED"* together with the sentence prescribing that a value there may only ever be set by OWNER PREFERENCE, provenance `owner`, never `validated:`; `regime.classifier_rule` lost its 2026-08-16 downgrade record, its survivorship bound, and *"THE PROJECT NOW HAS ZERO VALIDATED PARAMETERS"*. Nothing was wrong the day either note was written - both broke when a LATER edit appended a second key. **The danger is not limited to prose**: the same mechanism would drop a duplicated `value` or `provenance` and the system would silently use the second. Read through the YAML loader's own mapping constructor rather than by regex, so an indented `key:` inside a block scalar is not mistaken for one | **exists** - built 2026-09-01, found by an LLM council citing a note that no tool could see, and measured by planting a duplicate `value` and watching it go red (`AGENTS.md` §10.8) |
| 42 | `verify_control_characters.py` | a **C0 control byte** in a tracked text file — `BS`, `VT`, `FF`, `ESC` and the rest, with `TAB`, `LF` and `CR` deliberately absent. **exists** — built 2026-09-04, and it exists because a RULE was not enough. `AGENTS.md` §12 already forbids a backslash escape inside a heredoc and records what it cost: gate 14's widened pattern reached Python as a **backspace** instead of a word boundary, so the gate ran, printed *"0 failures"*, and could never have matched anything. The rule was read and broken again the morning this gate was built — a patch script piped through a heredoc turned `tools\verify_…` into a vertical tab and truncated five commands in `docs/runbooks/README.md`. **The gate checks the residue rather than the habit**, which is `AGENTS.md` §12's own standard for a check over text: an exact token, no prose parsed. Measured before shipping — 438 tracked text files, **zero** control bytes — so it ships with no exemption list and every future hit is real. **Its blind spot is stated rather than discovered**: a mangled edit that produced legal characters, such as a `\n` that became a real newline, is invisible to it |
| 43 | `verify_open_work.py` | a **closed** item left in `TODO.md`, which `AGENTS.md` §10.7 gives one job — *"every open item, and only open items"*. **exists** — built 2026-09-05 on the owner's instruction, after the file reached **4,653 lines of which 105 closed items occupied 2,557** — 57% of an open-work list that was not open work. The rule had said so since it was written and nothing could see it, because the rule was honour and the file is prose. **The subject is an exact token**, `- [x] ` at the start of a line, which is `CI_POLICY.md` §3's standard for a check over text: no English interpreted, no judgement made, and a structurally zero false-positive rate. Closed items go to `docs/08-pm/TODO_CLOSED.md`, and the discipline that travels with them is that the lesson is promoted first — to §12's traps, to a decision record, or to a gate — because an entry whose lesson has no other home is the only kind whose deletion loses something |
| 44 | `verify_minimum_effect.py` | a pre-registration that never says the smallest effect it could **detect**. **exists** — built 2026-09-07 on the owner's instruction, at trial 95 of a programme with **zero validated parameters**. `PR-012` refused a verdict for want of sample *after* spending its trials; `PR-013`'s six **gross** intervals all include zero; `PR-014`'s long-only intervals ran ±8 points, so a 3%-a-year edge was invisible to it and its `inconclusive` could not be told apart from *"there is nothing there"*. **A null from an instrument that could not have seen the effect is not evidence of absence, and it is not free** — `b.deflated_sharpe` counts every configuration evaluated, so such a study raises the hurdle for every study after it and can never repay it. **The subject is a token**, a line naming the number, in either the template's wording or `PR-015`'s *power floor*: the gate cannot read whether the number is RIGHT and does not try. Studies dated on or before 2026-09-07 are exempt and are **listed** — retro-fitting the line would edit a registered document or amend it after the run |
| 46 | `verify_window.py` | a study measuring a market that no longer exists and calling it more data. **exists** — built 2026-09-08 on the owner's instruction, *"not to measure back more than 48 month from now if its no real reason to measure old market instead of current"*. **Every study on the branch that raised it used nine to eleven years and none argued for it** — the window was the store's extent, which is an inventory rather than a reason. `AGENTS.md` §19.2 has the measured case and the sharpest part of it is that the admitted universe grew **from 38 names to 3,999** across the span those studies averaged over; survivorship also worsens with the lookback, on a store where **2,598 of 2,598** decade-long names still trade. **It reads the span the study OBTAINED**, never the one its arguments asked for — `PR-016` asked for 10.67 years and got 8.91 — and requires a `window rationale:` line past four years. **It cannot judge whether a reason is a good one**; §19.4 lists what holds and what does not, and that is a reviewer's job. Studies registered before 2026-09-08 are exempt and **listed by name**, and ten older results predate the `measured_span` field and are reported as one class rather than ten findings |
| 45 | `build_commands.py --check-only` | `README.md`'s command and flag reference drifting from the parsers that define it. **exists** — built 2026-09-07 on the owner's instruction to put every command and flag in the README. A hand-kept list of flags is wrong within the week, which is `AGENTS.md` §10.5's whole subject, so the block is **generated** from the argument parsers and this gate is what keeps it current — the same shape as gate 24 over `HANDOFF.md` §2. **It reads the SYNTAX TREE and imports nothing**: sixty-five tools declare a parser and a generator that imported them to ask what flags they take would be a generator that ran them. **What it cannot see it says rather than drops** — a flag whose name or help is computed appears with an empty description, which is how it surfaced eleven CLI arguments that carried no help text at all |
| 41 | `verify_vendor_policy.py` | the price vendor's network limits living anywhere but the committed policy — a timeout, a lookback ceiling or an `auto_adjust` literal in `market_data/`, a key missing from `registry/vendor_policy.yml`, or an adapter that never CALLS the loader. **exists** — gate 22's shape aimed at the dependency this project asks the most of: the broker and the symbol directory each had a committed policy while the price vendor, fetched about 2,300 times a day, had none. Reads the syntax tree, so the gate needs nothing installed |
...
| 3f | **A study census, overstating how many hypotheses had been refuted, in five documents at once.** The census belongs to `HANDOFF.md` §2 and is derived by `python tools/verify_study_summary.py`; the point here is the failure shape, not the figure — which is why the figure is no longer quoted, a numeral beside the word it counts being a live claim to that gate whatever the tense (`AGENTS.md` §10.5). One "study" is a post-hoc survivorship bound carrying no verdict at all. The evidence was right and every summary of it was wrong, in the direction of overstating how much had been tested. **The gate caught this class again on 2026-08-24**, when filing a new pre-registration exposed four documents still quoting the previous total — three of them spelling it in words, which gate 14 cannot see |
