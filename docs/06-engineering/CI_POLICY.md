...
| 6 | `lint-imports` | a package importing across a layer or forbidden boundary | **exists** — 5 contract subjects across 4 contracts. Caught a reversed layer order on first run. **And it was not being RUN: from the day the suite was wired until 2026-08-31 the runner invoked it as `python -m importlinter.cli lint-imports`, which imports a Click group with no `__main__` guard, falls off the end and exits 0** — so every suite run printed `PASS` over a check that never executed. Found by planting a forbidden import and watching it stay green (`AGENTS.md` §10.8). The tree turned out to be clean when the real runner was finally used — 4 kept, 0 broken — but that was luck, not enforcement. Now resolved as the console script beside the interpreter, in `check_gates._lint_imports` |
