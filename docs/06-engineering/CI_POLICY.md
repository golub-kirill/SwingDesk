...
| 34 | `verify_invariant_tests.py` | a test named in `INVARIANTS.md` §1 that could not fail. It copies `src/` to a scratch tree, breaks one invariant per committed mutant, and requires the named test to go red — 10 mutants over 7 of the 9 invariants, ~15s. **Written after the test named for invariant 1 was found to assert `(net / x) * x == net`**, an identity that held whatever the denominator contained. A mutation site that no longer matches is a failure rather than a skip, and the two invariants with no mutant are named on every run |
