...
| `PR-019` | On the entries the ratified screen selects, what holding period and stop width does the combination want - and does the best of them, chosen on the older market, survive the current one? | **registered** 2026-09-12 | **Owed by `STRATEGY_CONTRACT` C-7 and the owner's point that raised it: the hold must be researched and per strategy.** `exit.max_holding_period` is 20, `assumed:DR-012`, and no study has varied the per-trade hold on selected entries. Twelve cells - holds 10/20/40/60 x stops 2.0 ATR/4.0 ATR/none, no target - on ONE entry set spaced at 60 sessions so the pairing is exact. **The split buys selection**: one cell chosen in sample by a fixed rule, read only out of sample, which asks the owner's 2026-09-08 question directly - does the old market's best survive the current one. **First study sized by its own variance estimate before it registered** (`tools/power_pr019.py`), and the estimate predicts the answer's shape: precision falls with the hold, so a 40- or 60-session cell cannot be confirmed at the 0.15R floor, and `INCONCLUSIVE` there is written down as "cannot tell" before the data. 12 trials |
