...
| `PR-015` | Held for twenty sessions, long-only, does ANY of five selection signals earn a net excess over `SPY` whose interval excludes zero and replicates out of sample? | **registered** 2026-09-07, not yet run | **The first study here to change the SIGNAL.** `PR-013`, `PR-014` and every exploratory sweep hold one family — relative strength — and vary horizon, construction, universe or cost. `PR-014` removed horizon as the explanation: the ratified rule earns nothing that replicates at any hold from one month to a year. Five arms, one of them the incumbent as a control, one of them REVERSAL — the sign the literature documents inside this holding window and which nothing here has ever tested. Two holdouts at once, time AND names, because the backfill fixed the population problem and left the fact that every time window in this store is now spent. 5 trials |
