...
| `DR-035` | **`DR-027` §11 walks venue → book and cannot see the other way. A bracket's stop fires overnight, nothing records the exit, and the caps count a position that no longer exists — for ever. After four stop-outs the machine submits nothing again, silently.** | Run the full `reconcile` on the submission path. It already asks BOTH directions and already words this one — *"an exit that happened at the venue and was never recorded looks exactly like this"* — and `DR-027` §7 already ruled a divergence stop-submitting here. It was reachable from one caller: the `broker` command, which the scheduler never runs | none | **accepted 2026-09-03** |
| `DR-036` | **The owner asked what stops the open trades carried. The book had an answer for all three; the VENUE had none — every stop leg `canceled` and every target `expired` at the first close, because a bracket's legs inherit `time_in_force: day` while the position lives twenty sessions. Four separate checks could have noticed and none looks at the stop.** | Compare each position's `current_stop` against the venue's resting orders. No protection and a stop at the wrong price are reported separately, because a person acts on them differently. A submission stops: `risk.max_open_risk` is denominated in `entry − stop`, so a book whose stops are not standing bounds nothing. `broker` exits `3`. **Re-placing the stop is NOT done** — `DR-027` §2 leaves management actions to `D6`, and §4 lists the three options the owner has | none | **accepted 2026-09-03** |
| `DR-037` | **`DR-027` §3.2 argues a stop the market cannot see is not a stop; §3.3 gives every order `time_in_force: day`. A bracket's legs inherit the entry's, so the protection expires with the session while the position lives twenty of them. How does a stop last as long as what it protects?** | Keep the bracket `day` and place a separate **`gtc` OCO** once the position is recorded — as well as, not instead of, so the fill session is covered by the legs and every session after it by the OCO. `gtc` on the whole bracket was rejected by evidence: Alpaca takes `time_in_force` only on the parent, so it would reach the ENTRY, which is what §3.3 refused. A stop at the WRONG price is reported and never replaced — that is a move, and `D6` governs moves | none set: the stop is the book's own and the target is `exit.target_r_multiple` R above the recorded entry | **accepted 2026-09-03** |
| `DR-038` | **A position that ended at the venue could not be closed in the book by any shipped command: `closed_on` is written only for an `EXIT_NOW` proposed FROM BARS, so a stopped-out or taken-profit position held its cap slot for ever and stopped every submission after it. What may close it?** | The venue's **FILL**, never its silence. `sync-fills` closes a position when a SELL fill traces **by order id** to a `sent` submission of ours, recording the venue's share-weighted price and the last fill's session; absence closes nothing, because absence carries no price and a close with no price is a closed trade with no result. Refuses a partial, an oversell and a sell dated before the entry - each leaves the divergence standing, so `DR-027` §11 goes on stopping submission. `CHARTER` A-001 is not engaged: the trade already happened and writing it down is bookkeeping, the same argument `DR-031` makes for the opening half | none set | **accepted 2026-09-04** |
| `DR-039` | **The venue billed three regulatory fees on this project's first completed trade — REG, TAF and CAT — and not one of those categories exists anywhere in this repository. Meanwhile `costs.commission_model` charges `assumed:DR-004`'s 0.005/share for a commission Alpaca does not take, which on that trade is 3.4x the entire real fee bill. What does a trade actually cost at the venue?** | **A published formula, not an assumed rate.** Section 31 at **$20.60 per $1M of proceeds** and FINRA TAF at **$0.000166 per share**, both **sell-side only**, each rounded UP to the cent; commission goes to **zero**; CAT stays **unset** because $0.01 on one trade and $0.01 on twelve constrains no rate. Both formulas reproduce the billed cents exactly — which one observation can do because they are FUNCTIONS, not distributions, and the same observation says nothing about slippage. **A decision record rather than a pre-registration**: §8 governs a value that must be MEASURED, and these are PUBLISHED by the SEC and FINRA, so what they need is a citation. **Changes no verdict and is not offered as one** — the fees are 0.9% of `DR-005`'s slippage term. `DR-004`'s slippage and fill-price reasoning are untouched | `costs.commission_model`, `costs.regulatory_fee_schedule` | **accepted 2026-09-05** |
| `DR-040` | **`DR-004` ruled quoted spreads unobtainable and `DR-005` therefore set slippage from daily OHLC at 25.44 bps per side - the constant every negative headline in this project is computed at. The venue this project already holds an account with serves consolidated SIP NBBO free, back to 2016. What does the admitted universe actually cost to cross?** | **It depends on the minute, by a factor of six, and `DR-005` is RIGHT about the one the card trades in.** Measured against the venue's own NBBO over 2,208 windows, with the universe rebuilt on each sampled date: the median per-side spread at **09:30 is 21.9-30.2 bps** across five years - `DR-005` to within 0.8x-1.2x - and falls to **1.9-4.0 bps by 15:55**, where the same constant is **6.3x-13.6x** too high. `CARD-001` enters at `next session's open`, so the project charges and pays the worst value the session offers. Buy-and-hold turns positive below **20.6 bps per side** and the ratified exit cell below **6.2**; the measured close is **4.0**. **Proposes no number**: a later entry changes the GROSS as well as the cost, and the gross was measured at the open - subtracting a smaller cost from an unchanged gross is the error `DR-029` §5 made. What it proposes is that the model gain an execution-time dimension and that the constant record which moment it describes. **Spends no trial** - a cost input has no Sharpe to deflate | `costs.slippage_model` (shape and note only) | **accepted — ratified 2026-09-14** |
| `DR-041` | **The submission path has ten stop conditions and nine either recover on their own or correctly need a person. The tenth has no path at all: a stop RESTING at a price the book does not record. `restorable` deliberately leaves it alone - a second trigger on one position would apply a move nobody approved - so the run restores what it can, re-reads, still finds the position unprotected, and stops. Every evening. For ever. What should the book do about a number the venue is already holding?** | **Adopt a TIGHTER stop, refuse a WIDER one.** `DR-036` ruled that a stop the market cannot see is not a stop; this is its converse, and `reconcile.unprotected` already writes the argument - *"every R this position reports is denominated in the book's number, and the loss would be taken at the venue's"*. **The contract found the design error**: the first version adopted both directions and `ManagementAction` rejected it, `WIDE_STOP` - a trigger further from entry is a widening nobody approved, and recording it as APPROVED would launder a risk increase into the book and into every R. So the run still stops on that half, for a correct reason rather than a mechanical one. **Bookkeeping, not a management action**: nothing is sent and nothing is moved, the trigger is already resting there - the same argument `DR-031` makes for an opening and `DR-038` for a closing. **A LATENT deadlock, not a live one**: measured 2026-09-07, both open positions have their stops standing at the book's price and this has never fired | none | **accepted 2026-09-07** |
| `DR-042` | **`exit.target_r_multiple` was ruled 1R on 2026-09-01 and implemented on the LIVE path only. `ExitPolicy` carried two of the course's four exit slots — protective and time — so from that day every live order held a take-profit leg and every backtested trade did not. No study in this repository has ever run the exit this system places. And a daily bar records four prices and no times, so a session whose low reached the stop and whose high reached the target cannot say which came first — which leg does the harness take?** | **The profit slot goes into `ExitPolicy`, OPTIONAL so that `PR-002`, `PR-005`, `PR-011` and `PR-012` keep the policy their published trade logs were produced under. On an ambiguous bar the STOP is taken, always, and the bar is flagged so the count travels with every result.** Three arguments: it is the pessimistic reading; `backtesting.py` (kernc) resolves the same tie the same way and calls it an adversarial stance (authored import); and the venue's own answer is unknowable from a daily bar, so an assumption that can only understate is the one a research instrument should carry. **Seven mutations of the ordering, seven dead.** The ruling asked for is the tie-break alone — the count is reported precisely so it can be made on a number rather than on taste | none | **proposed 2026-09-07** |
| `DR-043` | **An approved stop move is written into the book and never reaches the venue: the venue keeps the old trigger, `DR-036` pauses entries, and a person cancels and re-places by hand - 13 stops placed by hand in five sessions for four positions, and on 2026-09-14 all four stood with no stop for the first 55 minutes of the session. Should the system send an approved move itself?** | **Proposed: replace, narrowly.** On `respond --approve` of a `MOVE_STOP` the system REPLACES its own protective stop's `stop_price` with the approved, higher one - one `PATCH`, no moment without a stop - and never a stop a person placed, never lower, only armed, journalled like every write. Cancel-then-place is rejected: the same effect with a window in the middle, and a queued cancel blocks the new stop for `insufficient qty`. **The interim needs no ruling and is built**: `swingdesk status` prints the exact commands | none - `PATCH` joins `allowed_methods` and leaves `REFUSED_METHODS` | **accepted — ratified 2026-09-14, option A** |
