...
| **C-3** | does the 4 ATR long-hold exit beat the MARKET over the same days, or is it drift | **`PR-019b` reported 2026-09-13 — `INCONCLUSIVE` for precision, and every reading points one way**: out of sample the candidate earned **−0.1543R [−0.2930, −0.0501] less a trade than `SPY` over the same sessions**, 0.243 wide against the 0.15R floor. Not retired — C-3 retires on `REJECT` or `NULL` — and **no evidence here supports it**: the index beat it in sample, at 3× costs and unscreened. Owed before the next such study: a power estimate that models overlapping holds |
