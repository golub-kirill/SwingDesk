...
| **C-3** | does the 4 ATR long-hold exit beat the MARKET over the same days, or is it drift | **`PR-019b` registered 2026-09-13** — `PR-019`'s no-selection null reads +0.0202R out of sample, which is what drift would look like. Pairing each trade with `SPY` over its own sessions predicts a half-width of 0.074R, where the trade alone reads 0.181R |
