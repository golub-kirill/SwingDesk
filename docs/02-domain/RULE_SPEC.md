...
## 0. What the form exists to make inexpressible

Three failure modes, each taken from the TradAlert post-mortem rather than from general principle.
The rest of this document is the machinery that forbids them.

| Failure | How the form forbids it |
|---|---|
| **the constant gate** — a rule whose verdict is invariant across all inputs | §6 requires a discriminating pair and a mutation check |
| **the silent `missing → value`** | §4's three-valued output and a mandatory `missing_data_policy` |
| **the decorative output** — a rule computed and consumed by nothing | mandatory `consumed_by` and `effect`, checked in §9 |

The first is not hypothetical, and §6 returns to it: TradAlert's R:R gate was `if is_long: return
True` and passed **seven audits**, because it is a valid function with valid references. Prose review
cannot catch it; only an executable test on a pair of inputs can.
