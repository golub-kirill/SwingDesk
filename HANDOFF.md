...
| Tests | **1267**, fully offline |
| Docs | 141 files, Tier 0-8 · indexed by `registry/project_manifest.yml` |
...
| Studies | 10 registered · 7 reported |
...
| | |
|---|---|
| Journal | 57 runs, 8 incomplete · **22 run(s) recorded against a dirty tree** and therefore not replayable from their SHA |
| Decisions | 48988 recorded · 0 uncoded refusals (`a.no_uncoded_failures` requires 0) |
| Bar store | 7,382,034 rows across 12,937 instruments |
| PIT integrity | **CLEAN** - bars whose `event_time` postdates their `knowledge_time`: 0 |
| Directory | **25 pulls** · **15 confirmed** against the response's own `Last-Modified` (`source_session_date`); of the rest, **7** predate the field and stay permanently unattributed (`DR-008` c3); **3** do NOT - they were taken after the field existed and the vendor file had not regenerated, so `DirectoryStore.record`'s monotonicity check dropped the claim. Each of those is a re-pull of an already-recorded session, which `DR-008` says should make **zero requests** |
| Universe coverage | bars stored for 12,937 of 13,187 listed symbols - **98.1%** |
...
| Track A clock | **3/20** consecutive clean sessions (2026-09-01 to 2026-09-03) · counting from a **deliberate restart on 2026-08-31**, not an outage - `python tools/track_a_streak.py` prints why · `a.run_completes`, computed by `tools/track_a_streak.py` |
...
- `claude/a-refusal-that-names-its-cause`
