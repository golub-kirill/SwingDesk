...
| | |
|---|---|
| Merge gates | **47**, one command: `python tools/check_gates.py` |
| Tests | **1261**, fully offline |
| Docs | 140 files, Tier 0-8 · indexed by `registry/project_manifest.yml` |
...
| | |
|---|---|
| Journal | 1 runs, 0 incomplete · **0 run(s) recorded against a dirty tree** and therefore not replayable from their SHA |
| Decisions | 0 recorded · 0 uncoded refusals (`a.no_uncoded_failures` requires 0) |
| Bar store | 251 rows across 1 instruments |
| PIT integrity | **CLEAN** - bars whose `event_time` postdates their `knowledge_time`: 0 |
| Directory | **0 pulls** · **0 confirmed** against the response's own `Last-Modified` (`source_session_date`); of the rest, none predate the field |
| Universe coverage | bars stored for 1 of 0 listed symbols - **n/a** |
| Canada | **nothing stored and nothing listed**. `BR-9`'s per-country requirement is unmet in every reported study. Since `DR-003` gap 1 was refuted (2026-08-25) a FORWARD result is blocked by this row rather than by a missing source; a HISTORICAL one also needs point-in-time membership, which the TMX endpoint cannot supply at any price |
| Classifications | 0 instrument(s) carry a sector · 0 (**n/a**) report at least one non-zero weight. The stricter `look_through` count, which also drops a degenerate ETF look-through (`DR-006` §8.7), is lower - derive it with `python tools/measure_sector_cap.py --wide --classifications data/classifications.duckdb` |
| Track A clock | UNAVAILABLE - no `data/daily_run.log` in this checkout |

*Measured from `data/` on 2026-09-04.*
...
- `claude/the-same-defect-in-three-more-places`
