...
| Tests | **1195**, fully offline |
| Docs | 138 files, Tier 0-8 · indexed by `registry/project_manifest.yml` |
...
| | |
|---|---|
| Journal | 46 runs, 8 incomplete · **20 run(s) recorded against a dirty tree** and therefore not replayable from their SHA |
| Decisions | 36425 recorded · 0 uncoded refusals (`a.no_uncoded_failures` requires 0) |
| Bar store | 3,616,406 rows across 3,750 instruments |
| PIT integrity | **CLEAN** - bars whose `event_time` postdates their `knowledge_time`: 0 |
| Directory | **24 pulls** · **14 confirmed** against the response's own `Last-Modified` (`source_session_date`); of the rest, **7** predate the field and stay permanently unattributed (`DR-008` c3); **3** do NOT - they were taken after the field existed and the vendor file had not regenerated, so `DirectoryStore.record`'s monotonicity check dropped the claim. Each of those is a re-pull of an already-recorded session, which `DR-008` says should make **zero requests** |
| Universe coverage | bars stored for 3,750 of 13,186 listed symbols - **28.4%** |
...
| Track A clock | **2/20** consecutive clean sessions (2026-09-01 to 2026-09-02) · counting from a **deliberate restart on 2026-08-31**, not an outage - `python tools/track_a_streak.py` prints why · `a.run_completes`, computed by `tools/track_a_streak.py` |

*Measured from `data/` on 2026-09-03.*
...
- `claude/an-exit-at-the-venue-is-not-invisible`
- `claude/handoff-2026-09-02`
